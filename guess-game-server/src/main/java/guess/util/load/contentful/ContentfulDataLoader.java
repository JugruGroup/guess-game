...
public class ContentfulDataLoader extends CmsDataLoader {
    ...
    public Map<String, List<String>> getTags(String conferenceCodePrefix) {
        ...
        for (ConferenceSpaceInfo conferenceSpaceInfo : ConferenceSpaceInfo.values()) {
            ...
            UriComponentsBuilder builder = UriComponentsBuilder
                    ...
                    .queryParam(ACCESS_TOKEN_PARAM_NAME, conferenceSpaceInfo.getAccessToken())
                    .queryParam(CONTENT_TYPE_PARAM_NAME, "talks")
                    .queryParam(SELECT_PARAM_NAME, conferenceSpaceInfo.getConferenceFieldName())
                    .queryParam(LIMIT_PARAM_NAME, MAXIMUM_LIMIT);

            if ((conferenceCodePrefix != null) && !conferenceCodePrefix.isEmpty()) {
                builder.queryParam(String.format("%s[match]", conferenceSpaceInfo.getConferenceFieldName()), conferenceCodePrefix);
            }

            var uri = builder
                    .buildAndExpand(conferenceSpaceInfo.getSpaceId(), ENTRIES_VARIABLE_VALUE)
                    .encode()
                    .toUri();
            ContentfulTalkResponse<? extends ContentfulTalkFields> response = getRestTemplate().getForObject(uri,
                    conferenceSpaceInfo.getTalkResponseClass());
    ...
    static List<Speaker> getSpeakers(ConferenceSpaceInfo conferenceSpaceInfo, String conferenceCode) {
        ...
        String additionalFieldNames = conferenceSpaceInfo.getSpeakerAdditionalFieldNames();
        ...
        UriComponentsBuilder builder = UriComponentsBuilder
                ...
                .queryParam(conferenceSpaceInfo.getSpeakerFlagFieldName(), "true")   // only speakers
                .queryParam(LIMIT_PARAM_NAME, MAXIMUM_LIMIT);

        if ((conferenceCode != null) && !conferenceCode.isEmpty()) {
            builder.queryParam(conferenceSpaceInfo.getConferenceFieldName(), conferenceCode);
        }
    ...
    static List<Talk> getTalks(ConferenceSpaceInfo conferenceSpaceInfo, String conferenceCode, boolean ignoreDemoStage) {
        ...
        String additionalFieldNames = conferenceSpaceInfo.getTalkAdditionalFieldNames();
